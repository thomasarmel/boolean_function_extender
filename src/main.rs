...
fn main() {
    // sac, 1st order CI, balanced, prop crit 2, prop crit 3, prop crit 4, prop crit 5
    ...
    (0..=(u32::MAX >> 1)).into_par_iter().for_each(|rule_number| {
        ...
        if U32Tester::is_strict_avalanche_criterion_ok(&rule_number) {
            *classes_eq_count[equivalent_class_index].3.lock().unwrap() += 1;
        }

        if U32Tester::is_first_order_correlation_immune(&rule_number) {
            *classes_eq_count[equivalent_class_index].4.lock().unwrap() += 1;
        }

        if U32Tester::is_function_balanced(&rule_number) {
            *classes_eq_count[equivalent_class_index].5.lock().unwrap() += 1;
        }

        if U32Tester::is_propagation_criterion_deg_k_ok(&rule_number, 2) {
        ...
        if U32Tester::is_propagation_criterion_deg_k_ok(&rule_number, 4) {
            *classes_eq_count[equivalent_class_index].8.lock().unwrap() += 1;
        }

        if U32Tester::is_propagation_criterion_deg_k_ok(&rule_number, 5) {
    ...
    for eq in &classes_eq_count {
        println!("{:#02x} -> {}", eq.0, *eq.3.lock().unwrap() * 2);
    }

    println!("-----------------------------------");
    println!("1st order CI:");
    for eq in &classes_eq_count {
        println!("{:#02x} -> {}", eq.0, *eq.4.lock().unwrap() * 2);
    }

    println!("-----------------------------------");
    println!("Balanced:");
    for eq in &classes_eq_count {
        println!("{:#02x} -> {}", eq.0, *eq.5.lock().unwrap() * 2);
    }

    println!("-----------------------------------");
    println!("Prop crit 2:");
    for eq in &classes_eq_count {
        println!("{:#02x} -> {}", eq.0, *eq.6.lock().unwrap() * 2);
    }

    println!("-----------------------------------");
    println!("Prop crit 3:");
    for eq in &classes_eq_count {
        println!("{:#02x} -> {}", eq.0, *eq.7.lock().unwrap() * 2);
    }

    println!("-----------------------------------");
    println!("Prop crit 4:");
    for eq in &classes_eq_count {
        println!("{:#02x} -> {}", eq.0, *eq.8.lock().unwrap() * 2);
    }

    println!("-----------------------------------");
    println!("Prop crit 5:");
    for eq in &classes_eq_count {
        println!("{:#02x} -> {}", eq.0, *eq.9.lock().unwrap() * 2);
    }
}
